...
*             图像初始化
*********************************/
uchar Image_Data[ROW][COLUMN]={0};    //图像数组
uchar RowData[ROW] = {0};
uchar ColData[COLUMN] = {0};
uchar VSYN_flg;            //场完成标志
uint_16 HREF_C;          //行数统计值  共240行
uchar Line_C;          //有效行计数 共60行
...
void SCI1_Transmit(void)
{
  ...
  for(i = 0;i< ROW;i++) 
  {
    ...
    { 
    //---------------发送二值化数据到SSCOM---------------------//
     //if(Image_Data[i][j]>THRESHOLD && Image_Data[i][j+1]>THRESHOLD && Image_Data[i][j-1]>THRESHOLD)  SCI1DRL = 0x30;
     //if(Image_Data[i][j]>THRESHOLD)  SCI1DRL = 0x30;
     // else SCI1DRL = 0X31;    //黑线
    //---------------发送二值化数据到SSCOM---------------------//
    
    //---------------发送像素数据到DEMOKTOOL---------------------//
      if(Image_Data[i][j] == 0xFF)  Image_Data[i][j] = 0xFE;  //若为图像头 自减
      SCI1DRL =  Image_Data[i][j];
      while (!(SCI1SR1&0x80)); 
     //---------------发送像素数据到DEMOKTOOL---------------------//
    }
...
/*****************************************************************************
 ...
 返 回 值  : NONE
*****************************************************************************/
void LED_P8x16Str(uchar ucIdxX, uchar ucIdxY, uchar ucDataStr[])
{
    uchar i, j, ucDataTmp;
...
{
    uint_16 i, j, k, usData;

    if(cData < 0)
    {
        LED_P6x8Char(ucIdxX, ucIdxY, '-');
        usData = (uint_16)(-cData);  
    }
    else
    {
        LED_P6x8Char(ucIdxX, ucIdxY, '+');
...
/*****************************************************************************
 ...
 输入参数  : uchar ucIdxX ucIdxX的范围为0～120
             ...
             int_16 sData  sData为需要转化显示的数值
 输出参数  : none
 返 回 值  : none
*****************************************************************************/
void LED_PrintShort(uchar ucIdxX, uchar ucIdxY, int_16 sData)
{
    uint_16 i, j, k, l, m, usData;  
    if(sData < 0)
    {
        LED_P6x8Char(ucIdxX,ucIdxY,'-');
        usData = (uint_16)(-sData);  
    }
    else
    {
        LED_P6x8Char(ucIdxX,ucIdxY,'+');
...
/*****************************************************************************
 ...
 输入参数  : uchar *pucTable     二维图像数组的地址
             uint_16 usRowNum    二维图像的行数1~64
             uint_16 usColumnNum 二维图像的列数1~128
 输出参数  : none
 返 回 值  : none
*****************************************************************************/
void LED_PrintImage(uchar *pucTable, uint_16 usRowNum, uint_16 usColumnNum)
{
    uchar ucData;
    uint_16 i,j,k,m,n;
    uint_16 usRowTmp;
